...
Sleep science and Neurology have recently achieved remarkable contributions to the understanding of the link between AD and sleep @Iliff12 @Xie13. From an endocrinological perspective, the lymphatic and glymphatic systems seem to act as a cleansing mechanism for the brain. Most importantly, these purification pipelines and channels seem to work at full steam only during sleep. A graphical summary of this chicken-and-the-egg causal chain of events that may take part in the Alzheimer/sleep interplay is shown in @loop. These recent findings have made it very hard for the scientific community to escape the conclusion that there is a tight connection between sleep patterns and the dawning (and development) of an AD case. In other words, the pathological states that go from sleep degradation to a potential AD scenario represent, in turn, a system of positive feedback loop @Doig18.
...
The approach presented in this work demonstrates, among other things, how it is possible to discretely monitor sleep in large AD-affected cohorts. Our implementation is based on the use of inertial sensors to track the patient's movements during the night. The underlying hardware is a comfortable wristband that continuously records accelerations, storing data (packed 32-bit values) on an SSD at a frequency of 50 Hz. Monitoring is performed without requiring connection to the power grid or the Internet, reinforcing the premise of privacy protection. Sleep periods are then estimated using the algorithm described in @Borazio14, but it basically consists in the following steps.

- To distinguish sleep periods from wakefulness, a low-pass filter is used, consisting of a sliding window (with threshold $k$) that identifies stillness events $Q S(t)$ of 1 second and labels them using the expression:
...
- Subsequently, all events with ($Q S(t) = 1$) are summed. Only the $i$ blocks lasting more than 10 minutes are labeled as deep sleep segments using $D S_i = [upright(l e n) (Q S ( t  = 1 ) > 600 med upright(s)]$, corresponding to Iverson's formula (1 if true, 0 if false).
...
People affected by AD (even at very early stages and even before being officially diagnosed) often feel disoriented and may therefore exhibit some degree of hostility. Daily informal reports by relatives and health professionals support this claim, as well as formal studies @Gallagher11, @Mussele13. For these reasons, there are very few research works that apply actigraphy hardware to the analysis of dementia. Perhaps the only studies related to inertial sleep monitoring in people suffering from dementia are @Andre19 and @Corbi22, where researchers suggest an interplay of modern, unobtrusive hardware as the perfect tool for sleep monitoring in adverse settings.

The simplicity of these systems makes them ideal for massive use in longitudinal and cross-sectional studies. With more detail, in @Andre19, a MotionWatch 8 wrist-worn triaxial device is used. This piece of equipment just records actigraphic data from cut-points applied to the activity counts throughout the daytime. The information is stored in an on-board memory for several days and is later read out and analyzed with special software (MotionWare). In the approach followed by @Corbi22, the system measures and stores raw acceleration.

/*
From these streams, periods of deep sleep and waking periods can be computed and other statistically representative values can be subsequently derived (@derived).
...
A total of ten AX3 devices are used (@ax3s). Each sensor is preassigned to a specific sub-cohort according to their known cognitive status, gender, and age-range.
